  ...
  return (
    <HiringLayout>
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3 mb-6">
        ...
        <button className="glass-button-primary px-4 py-2" onClick={() => setShowUploadArea((v) => !v)}>
          Upload New Resume
        </button>
      </div>
      ...
      {(showUploadArea || resumes.length === 0) && (
        <div className="glass-card border-2 border-dashed border-blue-500/30 p-10 text-center mb-6">
          ...
          <button className="glass-button-outline px-4 py-2">Browse Files</button>
        </div>
      )}

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
        {resumes.map((resume) => (
          <div key={resume.id} className="glass-card p-5 relative">
            ...
            <div className="flex flex-wrap gap-2">
              <button className="glass-button-outline px-3 py-1.5 text-sm inline-flex items-center gap-1"><Eye className="w-4 h-4" />View</button>
              <button className="glass-button-outline px-3 py-1.5 text-sm inline-flex items-center gap-1"><Download className="w-4 h-4" />Download</button>
              <button className="glass-button-primary px-3 py-1.5 text-sm" onClick={() => setPrimary(resume.id)}>Set as Default</button>
            </div>
          </div>
        ))}
      </div>
